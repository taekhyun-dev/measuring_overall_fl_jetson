...
def get_cifar10_loaders(sim_logger, num_clients, dirichlet_alpha, batch_size=128, data_root='./data', num_workers=0):
    """
    ...
    sim_logger.info(f"Created {num_clients} Non-IID clients with alpha={dirichlet_alpha}.")
    sim_logger.info(f"Total train samples: {train_size}, Validation samples: {val_size}")

    client_stats_df = pd.DataFrame(client_stats_list)
    client_stats_df.index.name = 'Client_ID'

    # --- 1. 표(Table)로 분포 확인 ---
    sim_logger.info("--- [Client Data Distribution (Table)] ---")
    sim_logger.info(client_stats_df)
